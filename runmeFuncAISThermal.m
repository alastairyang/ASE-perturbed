...
for steps = sim_steps
    ...
    if perform(org, 'PrescribeThermalBC') % {{{5 step 5 prescribe thermal boundary
        ...
        % thermal model: initial and boundary conditions
    ...
    if perform(org,'ThermalSteadyState') % {{{5 step 5 thermal steady state model
        ...
        md.basalforcings.geothermalflux = zeros(size(md.mesh.vertexonbase));
        md.basalforcings.geothermalflux(logical(md.mesh.vertexonbase)) = 80*1e-3;
        ...
        % do a long-term relaxation (stress-balance only)
        ...
        % save([project_data_dir 'saved-models/intermediate/ASE_before_thermal_relax.mat'],'md')

        % then do an adaptive time stepping
